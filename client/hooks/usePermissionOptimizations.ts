...
export const usePermissionOptimizations = (): UsePermissionOptimizationsReturn => {
  ...
  const applyOptimization = async (optimizationId: string) => {
    try {
      const response = await fetch(`/api/permissions/optimizations/${optimizationId}/apply`, {
        method: "POST",
      });
  ...
  const dismissOptimization = async (optimizationId: string) => {
    try {
      const response = await fetch(`/api/permissions/optimizations/${optimizationId}`, {
        method: "DELETE",
      });
      ...
      setError(err instanceof Error ? err.message : "Failed to dismiss optimization");
      throw err;
    }
  };
