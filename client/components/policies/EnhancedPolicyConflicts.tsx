...
export const EnhancedPolicyConflicts: React.FC<
  EnhancedPolicyConflictsProps
> = ({
  policies,
  roles,
  users,
  ...
  const [selectedConflict, setSelectedConflict] =
    useState<PolicyConflict | null>(null);
  ...
  const analyzePolicyConflicts = async () => {
    ...
    try {
      ...
      // 计算有效性指标
      const effectiveness = calculateEffectivenessMetrics(
        policies,
        allConflicts,
      );
      ...
      for (let j = i + 1; j < activePolicies.length; j++) {
        ...
        if (policy1.effect !== policy2.effect) {
          ...
          if (overlappingRules.length > 0) {
            ...
            const riskScore = calculateConflictRiskScore(
              policy1,
              policy2,
              overlappingRules,
              impactAnalysis,
            ...
            conflicts.push({
              ...
              suggestedResolution: generateResolutionSuggestion(
                "effect_conflict",
                policy1,
                policy2,
              ),
  ...
  const detectPriorityOverlaps = (policies: ABACPolicy[]): PolicyConflict[] => {
    ...
    priorityGroups.forEach((policiesInGroup, priority) => {
      if (policiesInGroup.length > 1) {
        const conflictingEffects = new Set(
          policiesInGroup.map((p) => p.effect),
        );

        if (conflictingEffects.size > 1) {
          const allowPolicies = policiesInGroup.filter(
            (p) => p.effect === "allow",
          );
          const denyPolicies = policiesInGroup.filter(
            (p) => p.effect === "deny",
          );
          ...
          if (allowPolicies.length > 0 && denyPolicies.length > 0) {
            const impactAnalysis: ImpactAnalysis = {
              ...
              affectedResources: [
                ...new Set(
                  policiesInGroup.flatMap((p) =>
                    p.rules.flatMap((r) =>
                      r.resource.map((res) => res.value as string),
                    ),
                  ),
                ),
              ],
  ...
  const detectRuleContradictions = (
    policies: ABACPolicy[],
  ): PolicyConflict[] => {
    const conflicts: PolicyConflict[] = [];
      ...
      for (let j = i + 1; j < activePolicies.length; j++) {
        ...
        if (overlapPercentage > 70) {
          ...
          const impactAnalysis: ImpactAnalysis = {
            ...
            affectedRoles: 0,
            affectedResources: [
              ...policy1.rules.flatMap((r) =>
                r.resource.map((res) => res.value as string),
              ),
              ...policy2.rules.flatMap((r) =>
                r.resource.map((res) => res.value as string),
              ),
            ],
            potentialSecurityRisk: "low",
  ...
  const attributeConditionsOverlap = (
  ...
  const calculateImpactAnalysis = (
    ...
    // 简化的影响分析
    const affectedRoles = roles.filter(
      (role) =>
        policy1.applicableRoles?.includes(role.id) ||
        policy2.applicableRoles?.includes(role.id),
    ).length;

    const affectedUsers = users.filter((user) =>
      user.roles.some(
        (roleId) =>
          policy1.applicableRoles?.includes(roleId) ||
          policy2.applicableRoles?.includes(roleId),
      ),
    ).length;
    ...
    return {
      ...
      potentialSecurityRisk:
        affectedUsers > 10 ? "high" : affectedUsers > 3 ? "medium" : "low",
      businessImpact:
  ...
  const determineConflictSeverity = (
    ...
    if (riskScore > 80 || impact.potentialSecurityRisk === "high")
      return "critical";
  ...
  const generateResolutionSuggestion = (
    conflictType: string,
    policy1: ABACPolicy,
    policy2: ABACPolicy,
  ): string => {
  ...
  const detectRuleContradiction = (
    ...
    const sameResource = rule1.resource.some((r1) =>
      rule2.resource.some(
        (r2) => r1.attribute === r2.attribute && r1.value === r2.value,
      ),
    );
  ...
  const checkPolicyRoleAmbiguity = (
    policy: ABACPolicy,
    role: Role,
  ): string[] => {
    // 简化的歧义检测
  ...
  const areTimeConstraintsConflicting = (
    constraint1: any,
    constraint2: any,
  ): boolean => {
    // 简化的时间约束冲突检测
    if (
      constraint1.type === "time_window" &&
      constraint2.type === "time_window"
    ) {
  ...
  const calculateConditionOverlap = (
    policy1: ABACPolicy,
    policy2: ABACPolicy,
  ): number => {
    // 简化的条件重叠计算
    ...
    return totalConditions > 0
      ? (overlappingConditions / totalConditions) * 100
      : 0;
  };
    ...
    for (let i = 0; i < activePolicies.length; i++) {
      for (let j = i + 1; j < activePolicies.length; j++) {
        ...
        if (Math.abs(policy1.priority - policy2.priority) <= 10)
          overlapTypes.push("priority");
        if (overlapPercentage > 50) overlapTypes.push("conditions");

        const conflictRisk =
          overlapPercentage *
          (overlapTypes.length / 3) *
          (policy1.effect !== policy2.effect ? 1.5 : 1);
  ...
  const calculateEffectivenessMetrics = (
    policies: ABACPolicy[],
    conflicts: PolicyConflict[],
  ): PolicyEffectivenessMetrics[] => {
    return policies.map((policy) => {
      const policyConflicts = conflicts.filter((c) =>
        c.involvedPolicies.includes(policy.id),
      );
      const conflictRate = policyConflicts.length;

      // 简化的指标计算
      const activationRate =
        policy.status === "active" ? 85 + Math.random() * 15 : 0;
      const performanceImpact =
        policy.rules.length * 2 + (policy.evaluationMode === "strict" ? 5 : 0);
  ...
  const calculateAnalysisMetrics = (
    ...
    const typeDistribution = {
      effect_conflict: conflicts.filter((c) => c.type === "effect_conflict")
        .length,
      priority_overlap: conflicts.filter((c) => c.type === "priority_overlap")
        .length,
      rule_contradiction: conflicts.filter(
        (c) => c.type === "rule_contradiction",
      ).length,
      scope_ambiguity: conflicts.filter((c) => c.type === "scope_ambiguity")
        .length,
      temporal_conflict: conflicts.filter((c) => c.type === "temporal_conflict")
        .length,
      condition_overlap: conflicts.filter((c) => c.type === "condition_overlap")
        .length,
    };

    const avgRiskScore =
      conflicts.length > 0
        ? conflicts.reduce((sum, c) => sum + c.riskScore, 0) / conflicts.length
        : 0;
    ...
    return {
      ...
      resolutionRate:
        totalConflicts > 0
          ? (autoResolvableConflicts / totalConflicts) * 100
          : 0,
    };
  };
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span>解决率</span>
                          <span>
                            {analysisMetrics.resolutionRate?.toFixed(1)}%
                          </span>
                        </div>
                        <Progress
                          value={analysisMetrics.resolutionRate || 0}
                          className="h-2"
                        />
                      </div>
                    </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {Object.entries(analysisMetrics.typeDistribution).map(
                        ([type, count]) => (
                          <div
                            key={type}
              ...
              <div className="space-y-3">
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="grid grid-cols-3 gap-4 text-xs text-gray-500">
                                <div>
                                  <span className="font-medium">影响用户:</span>{" "}
                                  {conflict.impactAnalysis.affectedUsers}
                                </div>
                                <div>
                                  <span className="font-medium">影响角色:</span>{" "}
                                  {conflict.impactAnalysis.affectedRoles}
                                </div>
                                <div>
                                  <span className="font-medium">涉及资源:</span>{" "}
                                  {
                                    conflict.impactAnalysis.affectedResources
                                      .length
                                  }
                                </div>
                              </div>
                        ...
                        <CardContent className="p-4 pt-0 border-t bg-gray-50">
                          <div className="space-y-4">
                            ...
                            {conflict.conflictingRules &&
                              conflict.conflictingRules.length > 0 && (
                                <div>
                                  <h4 className="font-medium mb-2">冲突规则</h4>
                                  <div className="space-y-2">
                                    {conflict.conflictingRules.map(
                                      (rule, index) => (
                                        <div
                                          key={index}
                ...
                {filteredConflicts.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500">所有ABAC策略配置都符合规范</p>
                  </div>
                )}
              </div>
            </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {overlapMatrix.slice(0, 20).map((overlap, index) => {
                        const policy1 = policies.find(
                          (p) => p.id === overlap.policy1Id,
                        );
                        const policy2 = policies.find(
                          (p) => p.id === overlap.policy2Id,
                        );
                        ...
                        return (
                          <TableRow key={index}>
                            ...
                            <TableCell>
                              <Badge
                                className={
                                  overlap.conflictRisk > 75
                                    ? "bg-red-100 text-red-800"
                                    : overlap.conflictRisk > 50
                                      ? "bg-orange-100 text-orange-800"
                                      : overlap.conflictRisk > 25
                        ...
                        return (
                          <TableRow key={metrics.policyId}>
                            ...
                            <TableCell>
                              <Badge
                                className={
                                  metrics.conflictRate > 3
                                    ? "bg-red-100 text-red-800"
                                    : metrics.conflictRate > 1
                                      ? "bg-orange-100 text-orange-800"
                                      : "bg-green-100 text-green-800"
                                }
                              >
                  ...
                  <div className="space-y-4">
                    ...
                    {analysisMetrics?.typeDistribution.effect_conflict > 0 && (
                      <Alert>
                        ...
                        <AlertDescription>
                          <strong>效果冲突:</strong> 检测到{" "}
                          {analysisMetrics.typeDistribution.effect_conflict}{" "}
                          个allow/deny效果冲突。建议审查策略优先级设置。
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    {analysisMetrics?.avgRiskScore > 60 && (
                      <Alert>
                        <AlertTriangle className="h-4 w-4" />
                        <AlertDescription>
                          <strong>高风险评分:</strong> 平均冲突风险评分为{" "}
                          {analysisMetrics.avgRiskScore}
                          ，建议进行策略优化以降低系统风险。
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    {analysisMetrics?.resolutionRate < 50 && (
                      <Alert>
                        ...
                        <AlertDescription>
                          <strong>自动解决率低:</strong> 只有{" "}
                          {analysisMetrics.resolutionRate.toFixed(1)}%
                          的冲突可以自动解决。考虑重新设计策略以提高可维护性。
                        </AlertDescription>
                      </Alert>
                    )}
