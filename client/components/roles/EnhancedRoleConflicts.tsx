...
export const EnhancedRoleConflicts: React.FC<EnhancedRoleConflictsProps> = ({
  ...
  const [selectedConflict, setSelectedConflict] = useState<RoleConflict | null>(
    null,
  );
  const [circularDependencies, setCircularDependencies] = useState<
    CircularDependency[]
  >([]);
  const [hierarchyConflicts, setHierarchyConflicts] = useState<
    HierarchyConflict[]
  >([]);
      ...
      if (role.parentRole) {
        ...
        if (parent) {
          const childExtraPermissions = role.permissions.filter(
            (perm) =>
              !parent.permissions.includes(perm) &&
  ...
  const analyzeConflictPatterns = (
    ...
    const typeDistribution = {
      permission_overlap: conflicts.filter(
        (c) => c.type === "permission_overlap",
      ).length,
      hierarchy_violation: conflicts.filter(
        (c) => c.type === "hierarchy_violation",
      ).length,
      separation_of_duties: conflicts.filter(
    ...
    conflicts.forEach((conflict) => {
      conflict.roles.forEach((roleId) => {
        users
          .filter((u) => u.roles.includes(roleId))
          .forEach((u) => affectedUsers.add(u.id));
      });
    });
  ...
  const calculateRiskScore = (
    ...
    const severityScore = Object.entries(severity).reduce(
      (score, [level, count]) =>
        score + weights[level as keyof typeof weights] * (count as number),
      0,
    );
  ...
  const generateRecommendations = (severity: any, types: any): string[] => {
    ...
    if (severity.critical > 0) {
      recommendations.push(
        `立即解决 ${severity.critical} 个严重冲突，这些可能导致安全漏洞`,
      );
    }

    if (types.hierarchy_violation > 0) {
      recommendations.push(
        `检查角色层级结构，修复 ${types.hierarchy_violation} 个层级违规`,
      );
    }

    if (types.separation_of_duties > 0) {
      recommendations.push(
        `审查职责分离策略，处理 ${types.separation_of_duties} 个冲突`,
      );
    }
  ...
  const generateAutoResolutionActions = (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge
                  className={
                    (analysisResults?.riskScore || 0) > 75
                      ? "bg-red-100 text-red-800"
                      : (analysisResults?.riskScore || 0) > 50
                        ? "bg-orange-100 text-orange-800"
                        : (analysisResults?.riskScore || 0) > 25
    ...
    <Card>
      ...
      <CardContent>
        <Tabs defaultValue="overview" className="space-y-4">
          ...
          <TabsContent value="overview">{renderAnalysisOverview()}</TabsContent>
            ...
            <div className="space-y-4">
              ...
              {conflicts.filter((c) => !c.resolved).length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">所有角色分配都符合系统规则</p>
                </div>
              )}
            </div>
          </TabsContent>
