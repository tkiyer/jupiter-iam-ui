...
const businessPolicies: ABACPolicy[] = [
  {
    ...
    name: '费用审批分级策略',
    description: '基于用户角色和费用金额的分级审批策略，体现RBAC基础权限+ABAC动态条件',
    rules: [
      {
        subject: [
          { attribute: 'user_roles', operator: 'in', value: ['employee'] },
          { attribute: 'department', operator: 'equals', value: '${resource.department}' }
        ],
        resource: [
  ...
  },
  {
    ...
    name: '部门经理审批策略',
    description: '部门经理可审批本部门5万以下费用，展示角色权限与业务上下文结合',
    rules: [
      {
        subject: [
          { attribute: 'user_roles', operator: 'in', value: ['department_manager'] },
          { attribute: 'department', operator: 'equals', value: '${resource.expense_department}' },
          { attribute: 'employment_status', operator: 'equals', value: 'active' }
        ],
        resource: [
          { attribute: 'type', operator: 'equals', value: 'expense_request' },
          { attribute: 'amount', operator: 'less_than', value: 50000 },
          { attribute: 'status', operator: 'equals', value: 'pending_approval' }
        ],
  ...
  },
  {
    ...
    rules: [
      {
        subject: [
          { attribute: 'user_roles', operator: 'in', value: ['sales_rep', 'sales_manager'] },
          { attribute: 'assigned_region', operator: 'equals', value: '${resource.customer_region}' },
          { attribute: 'sales_certification', operator: 'equals', value: 'valid' }
        ],
        resource: [
          { attribute: 'type', operator: 'equals', value: 'customer_data' },
          { attribute: 'data_classification', operator: 'not_equals', value: 'highly_confidential' }
        ],
        action: ['read', 'update_notes', 'schedule_meeting'],
        environment: [
          { attribute: 'access_location', operator: 'in', value: ['office', 'authorized_remote'] },
    ...
    rules: [
      {
        ...
        resource: [
          ...
          { attribute: 'contains_salary_info', operator: 'equals', value: true }
        ],
  ...
  },
  {
    ...
    rules: [
      {
        subject: [
          { attribute: 'user_roles', operator: 'in', value: ['devops_engineer', 'senior_engineer'] },
          { attribute: 'deployment_certification', operator: 'equals', value: 'current' },
          { attribute: 'on_call_duty', operator: 'equals', value: true }
        ],
        resource: [
          { attribute: 'environment', operator: 'equals', value: 'production' },
          { attribute: 'change_risk_level', operator: 'in', value: ['low', 'medium'] }
        ],
  ...
  },
  {
    ...
    rules: [
      {
        subject: [
          { attribute: 'user_roles', operator: 'in', value: ['manager', 'senior_staff'] },
          { attribute: 'emergency_contact', operator: 'equals', value: true }
        ],
        resource: [
          { attribute: 'emergency_access_required', operator: 'equals', value: true }
        ],
        action: ['emergency_override', 'temporary_escalation'],
        environment: [
          { attribute: 'incident_level', operator: 'in', value: ['critical', 'high'] },
          { attribute: 'emergency_declared', operator: 'equals', value: true },
          { attribute: 'security_officer_notified', operator: 'equals', value: true }
        ]
      }
    ],
    effect: 'allow',
  ...
  },
  {
    ...
    rules: [
      {
        subject: [
          { attribute: 'user_roles', operator: 'in', value: ['finance_manager', 'cfo', 'ceo'] },
          { attribute: 'department', operator: 'in', value: ['finance', 'executive'] },
          { attribute: 'financial_clearance', operator: 'greater_than', value: 2 }
        ],
        resource: [
          { attribute: 'type', operator: 'equals', value: 'financial_report' },
          { attribute: 'report_category', operator: 'in', value: ['quarterly', 'annual'] },
          { attribute: 'confidentiality_level', operator: 'in', value: ['confidential', 'restricted'] }
        ],
        action: ['read', 'analyze', 'share_with_board'],
        environment: [
          { attribute: 'network_location', operator: 'equals', value: 'corporate' },
          { attribute: 'device_compliance', operator: 'equals', value: 'compliant' }
        ]
      }
    ],
    effect: 'allow',
    ...
    rules: [
      {
        subject: [
          ...
          { attribute: 'contract_end_date', operator: 'greater_than', value: '${current_date}' }
        ],
        resource: [
  ...
  },
  {
    ...
    rules: [
      {
        subject: [
          { attribute: 'user_roles', operator: 'in', value: ['admin', 'system_admin'] }
        ],
        resource: [
          { attribute: 'operation_risk', operator: 'in', value: ['high', 'critical'] }
        ],
        action: ['delete_user', 'modify_permissions', 'system_shutdown', 'data_purge'],
        environment: [
          { attribute: 'day_of_week', operator: 'in', value: ['saturday', 'sunday'] },
          { attribute: 'emergency_override', operator: 'not_equals', value: true }
        ]
      }
    ],
    effect: 'deny',
  ...
  },
  {
    ...
    rules: [
      {
        subject: [
          { attribute: 'department', operator: 'not_equals', value: '${resource.data_owner_department}' },
          { attribute: 'cross_department_approval', operator: 'equals', value: true }
        ],
        resource: [
          { attribute: 'department_restricted', operator: 'equals', value: true },
          { attribute: 'data_sensitivity', operator: 'greater_than', value: 2 }
        ],
        action: ['read', 'access'],
        environment: [
          { attribute: 'approval_valid_until', operator: 'greater_than', value: '${current_timestamp}' },
          { attribute: 'approver_department', operator: 'equals', value: '${resource.data_owner_department}' }
        ]
      }
    ],
    effect: 'allow',
...
const businessRulesEngine = {
  ...
  expenseApprovalRules: {
    ...
    checkDepartmentMatch: (userDepartment: string, expenseDepartment: string) => {
  ...
  customerAccessRules: {
    ...
    checkDataClassificationAccess: (userRole: string, dataClassification: string) => {
      const accessMatrix = {
  ...
  hrAccessRules: {
    ...
    validateCrossDepartmentAccess: (userDept: string, targetDept: string, approvalRequired: boolean) => {
...
export const handleTestBusinessScenario: RequestHandler = (req, res) => {
  try {
    const { scenario_type, user_context, resource_context, action, policies_to_test } = req.body;
    ...
    const applicablePolicies = policies_to_test?.length > 0 
    ...
    const evaluationResults = applicablePolicies.map(policy => {
      ...
      const businessRulesResult = evaluateBusinessRules(policy, testContext, action);
      ...
      return {
        ...
        final_decision: combinePolicyResults(rbacResult, abacResult, businessRulesResult, policy.effect),
    ...
    res.json({
      ...
      business_logic_explanation: generateBusinessLogicExplanation(evaluationResults, finalDecision),
...
export const handleIntegrationAnalysis: RequestHandler = (req, res) => {
  try {
    const analysis = {
      ...
      rbac_only_policies: businessPolicies.filter(p => hasOnlyRBACRules(p)).length,
      abac_only_policies: businessPolicies.filter(p => hasOnlyABACRules(p)).length,
      integrated_policies: businessPolicies.filter(p => hasIntegratedRules(p)).length,
      ...
      complexity_metrics: {
        average_rules_per_policy: (businessPolicies.reduce((sum, p) => sum + p.rules.length, 0) / businessPolicies.length).toFixed(2),
...
function analyzeRBACandABACIntegration(policy: ABACPolicy) {
  return {
    ...
    has_attribute_conditions: policy.rules.some(rule => 
      rule.subject.some(cond => cond.attribute !== 'user_roles') ||
      rule.resource.length > 0 ||
      (rule.environment && rule.environment.length > 0)
    ),
    integration_complexity: policy.rules.reduce((sum, rule) => 
...
function generateIntegrationSummary(policies: any[]) {
  return {
    ...
    average_complexity: (policies.reduce((sum, p) => sum + p.complexity_score, 0) / policies.length).toFixed(1),
    integration_distribution: {
      rbac_heavy: policies.filter(p => p.integration_analysis.has_role_conditions && !p.integration_analysis.has_attribute_conditions).length,
      abac_heavy: policies.filter(p => !p.integration_analysis.has_role_conditions && p.integration_analysis.has_attribute_conditions).length,
      integrated: policies.filter(p => p.integration_analysis.has_role_conditions && p.integration_analysis.has_attribute_conditions).length
    }
  };
}

function evaluateRBACComponent(policy: ABACPolicy, context: any, action: string) {
...
function evaluateABACComponent(policy: ABACPolicy, context: any, action: string) {
  ...
  policy.rules.forEach(rule => {
    ...
    rule.subject.filter(cond => cond.attribute !== 'user_roles').forEach(cond => {
      totalConditions++;
  ...
  return {
    ...
    reason: passed ? '所有属性条件均满足' : `${passedConditions}/${totalConditions} 属性条件满足`
  };
}

function evaluateBusinessRules(policy: ABACPolicy, context: any, action: string) {
...
function evaluateExpenseApprovalRules(context: any, action: string) {
  ...
  const userRoles = context.subject.user_roles || [];
  const requiredApprover = businessRulesEngine.expenseApprovalRules.getRequiredApprover(
    amount, context.subject.department
  );
  ...
  if (action === 'submit' && userRoles.includes('employee')) {
    ...
    if (requiredApprover === 'department_manager' && userRoles.includes('department_manager')) {
      passed = businessRulesEngine.expenseApprovalRules.checkDepartmentMatch(
        context.subject.department, context.resource.expense_department
      );
  ...
  return {
    ...
    passed,
    reason,
    business_rules_applied: ['amount_threshold_check', 'department_match_validation']
  };
}
...
function evaluateCustomerAccessRules(context: any, action: string) {
  ...
  const regionMatch = businessRulesEngine.customerAccessRules.validateRegionAccess(userRegion, customerRegion);
  const dataAccess = businessRulesEngine.customerAccessRules.checkDataClassificationAccess(userRole, dataClassification);
  ...
  return {
    evaluated: true,
    passed,
    reason: passed ? '区域和数据分类访问验证通过' : '区域不匹配或数据分类权限不足',
...
function evaluateHRAccessRules(context: any, action: string) {
  ...
  if (containsSalaryInfo) {
    const accessLevel = businessRulesEngine.hrAccessRules.getSalaryAccessLevel(userRole, clearanceLevel);
    const passed = accessLevel !== 'none';

    return {
      evaluated: true,
      passed,
      reason: passed ? `具有${accessLevel}级别薪资信息访问权限` : '缺少薪资信息访问权限',
...
function combinePolicyResults(rbacResult: any, abacResult: any, businessResult: any, effect: string) {
  const allPassed = rbacResult.passed && abacResult.passed && businessResult.passed;
...
function generateFinalAccessDecision(evaluationResults: any[]) {
  ...
  const sortedResults = evaluationResults.sort((a, b) => b.priority - a.priority);
...
function generateBusinessLogicExplanation(evaluationResults: any[], finalDecision: any) {
  return {
...
function identifyBusinessLogicElements(policy: ABACPolicy) {
  ...
  policy.rules.forEach(rule => {
    if (rule.subject.some(c => c.attribute === 'department')) elements.push('department_based_control');
    if (rule.resource.some(c => c.attribute === 'amount')) elements.push('amount_threshold_control');
    if (rule.environment?.some(c => c.attribute === 'time')) elements.push('time_based_control');
    if (rule.environment?.some(c => c.attribute === 'location')) elements.push('location_based_control');
  });
...
function calculateAverageConditionsPerRule(): string {
  const totalConditions = businessPolicies.reduce((sum, policy) => 
    sum + policy.rules.reduce((ruleSum, rule) => 
      ruleSum + rule.subject.length + rule.resource.length + (rule.environment?.length || 0), 0), 0);
  
  const totalRules = businessPolicies.reduce((sum, policy) => sum + policy.rules.length, 0);
  
  return (totalConditions / totalRules).toFixed(2);
}

function calculateOverallIntegrationComplexity(): number {
  return businessPolicies.reduce((sum, policy) => sum + calculateComplexityScore(policy), 0);
}
