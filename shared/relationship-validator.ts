...
import { User, Role, Permission, ABACPolicy, PolicyRule, AttributeCondition } from './iam';
...
export interface ValidationError {
  type: 'circular_dependency' | 'conflicting_permissions' | 'invalid_assignment' | 'policy_conflict';
  message: string;
  entityIds: string[];
...
export interface ValidationWarning {
  type: 'redundant_assignment' | 'over_privileged' | 'under_privileged' | 'policy_overlap';
  message: string;
  entityIds: string[];
  suggestion?: string;
}
...
export class RelationshipValidator {
  ...
  static validateUserRoleAssignment(
    ...
    // 检查角色冲突
    const conflictingRoles = this.findConflictingRoles(roleId, user.roles, roles);
  ...
  static validateRolePermissionAssignment(
    ...
    // 验证权限是否存在
    const invalidPermissions = permissionIds.filter(pid => 
    ...
    // 检查权限冲突
    const conflictingPermissions = this.findConflictingPermissions(permissionIds, permissions);
    ...
    const duplicatePermissions = permissionIds.filter(pid => existingPermissions.includes(pid));
  ...
  static validatePolicyConfiguration(
    ...
    // 检查策略名称冲突
    const nameConflict = existingPolicies.find(p => 
      p.id !== policy.id && p.name === policy.name
    );
    if (nameConflict) {
      errors.push({
        type: 'invalid_assignment',
    ...
    // 检查策略冲突
    const conflictingPolicies = this.findConflictingPolicies(policy, existingPolicies);
  ...
  static detectSystemConflicts(
    ...
    // 检测权限冲突
    const permissionConflicts = this.detectPermissionConflicts(permissions, roles);
  ...
  private static findConflictingRoles(roleId: string, existingRoles: string[], roles: Role[]): string[] {
  ...
  private static areRolesConflicting(role1: Role, role2: Role): boolean {
    // 检查是否有互斥标记
    if (role1.name.includes('admin') && role2.name.includes('guest')) return true;
    if (role1.name.includes('read') && role2.name.includes('write')) return false; // 读写不冲突
  ...
  private static findConflictingPermissions(permissionIds: string[], permissions: Permission[]): string[] {
    const conflicts: string[] = [];
  ...
  private static arePermissionsConflicting(perm1: Permission, perm2: Permission): boolean {
    // 检查读写冲突（在某些特定资源上）
    if (perm1.scope === perm2.scope && 
        perm1.name.includes('delete') && perm2.name.includes('read_only')) {
      return true;
    }
  ...
  private static findConflictingPolicies(policy: ABACPolicy, existingPolicies: ABACPolicy[]): ABACPolicy[] {
    return existingPolicies.filter(existingPolicy => {
      if (existingPolicy.id === policy.id) return false;
      
      // 检查优先级相同但���果相反的策略
      if (existingPolicy.priority === policy.priority && 
  ...
  private static havePolicyRuleOverlap(policy1: ABACPolicy, policy2: ABACPolicy): boolean {
    // 简化的重叠检测逻辑
    return policy1.rules.some(rule1 => 
      policy2.rules.some(rule2 => 
        this.doRulesOverlap(rule1, rule2)
      )
    );
  }
  ...
  private static doConditionsOverlap(cond1: AttributeCondition, cond2: AttributeCondition): boolean {
    return cond1.attribute === cond2.attribute && 
  ...
  private static detectPermissionConflicts(permissions: Permission[], roles: Role[]): Conflict[] {
    const conflicts: Conflict[] = [];
    ...
    roles.forEach(role => {
      ...
      for (let i = 0; i < rolePermissions.length; i++) {
        for (let j = i + 1; j < rolePermissions.length; j++) {
          if (this.arePermissionsConflicting(rolePermissions[i], rolePermissions[j])) {
            conflicts.push({
              ...
              involvedEntities: [role.id, rolePermissions[i].id, rolePermissions[j].id],
              severity: 'medium',
              resolution: '移除冲突的权限'
            });
          }
  ...
  private static detectPolicyConflicts(policies: ABACPolicy[]): Conflict[] {
    ...
    for (let i = 0; i < policies.length; i++) {
      for (let j = i + 1; j < policies.length; j++) {
        if (policies[i].effect !== policies[j].effect && 
